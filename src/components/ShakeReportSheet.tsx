 ...
 *
 * Auto-attaches:
 *   - module_context (current React Navigation route → PM area)
 *   - environment (platform, version, viewport, company_id, ...)
 *
 * Optional: pick a screenshot from the photo library; we encode it as a
 * data-url and ship it inline in `environment.attachments_inline`.
 */
...
export function ShakeReportSheet({
  ...
  const detectedModule = detectProjeXtPalMobileModule(routeName);
  ...
  // Lazy-load company once when the sheet first becomes visible.
  useEffect(() => {
    if (!visible || companyId !== null) return;
    void getCurrentCompanyId().then((id) => setCompanyId(id));
  }, [visible, companyId]);
  ...
  async function submit() {
    if (!companyId) {
      Alert.alert(
        ...
        'Login eerst en zorg dat je bij een bedrijf hoort.'
      );
      return;
    }
    if (!title.trim()) {
    ...
    setSubmitting(true);
    try {
      const env = captureMobileEnvironment(routeName, companyId);
        ...
        >
          ...
          >
            ...
            <TextInput
              ...
              placeholder="Bijv. 'Risico kan niet opgeslagen worden'"
            ...
            <View style={styles.chipRow}>
              {CATEGORY_OPTIONS.map((opt) => {
                ...
                return (
                  ...
                  >
                    {active ? <MaterialIcons name="check" size={12} color="#fff" /> : null}
                    <Text
            ...
            <TouchableOpacity onPress={pickScreenshot} style={styles.pickerButton}>
              <MaterialIcons name="camera-alt" size={14} color="#7c3aed" />
            ...
            <View style={styles.envInfoBox}>
              <Text style={styles.envInfoText}>
                ...
                schermresolutie en bedrijf — voor sneller reproduceren.
              </Text>
            </View>
