...
export default function SessionsCard() {
  ...
  // Only genuinely usable sessions: not revoked and not expired.
  const now = Date.now();
  const active = sessions.filter((s) => !s.revoked && (!s.expires_at || new Date(s.expires_at).getTime() > now));
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {active.length > 0 && (
          <p className="text-xs text-muted-foreground pt-1">{active.length} {pt("active session(s)")}</p>
        )}
        {active.length > 1 && (
