  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">{pt("Admin Dashboard")}</h1><p className="text-sm text-muted-foreground">{pt("System overview and statistics")}</p></div>
        <Button variant="outline" onClick={fetchStats} className="gap-2"><RefreshCw className="h-4 w-4" /> {pt("Refresh")}</Button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardContent className="p-4 flex items-center gap-4"><div className="h-12 w-12 rounded-lg bg-blue-100 flex items-center justify-center"><Users className="h-6 w-6 text-blue-600" /></div><div><p className="text-sm text-muted-foreground">{pt("Total Users")}</p><p className="text-2xl font-bold">{ov.total_users ?? s.total_users ?? 0}</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-4"><div className="h-12 w-12 rounded-lg bg-green-100 flex items-center justify-center"><Building2 className="h-6 w-6 text-green-600" /></div><div><p className="text-sm text-muted-foreground">{pt("Organizations")}</p><p className="text-2xl font-bold">{ov.total_companies ?? s.total_companies ?? 0}</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-4"><div className="h-12 w-12 rounded-lg bg-purple-100 flex items-center justify-center"><CreditCard className="h-6 w-6 text-purple-600" /></div><div><p className="text-sm text-muted-foreground">{pt("Active Subscriptions")}</p><p className="text-2xl font-bold">{ov.active_subscriptions ?? s.active_subscriptions ?? 0}</p></div></CardContent></Card>
        <Card><CardContent className="p-4 flex items-center gap-4"><div className="h-12 w-12 rounded-lg bg-orange-100 flex items-center justify-center"><Activity className="h-6 w-6 text-orange-600" /></div><div><p className="text-sm text-muted-foreground">{pt("Projects")}</p><p className="text-2xl font-bold">{ov.total_projects ?? s.total_projects ?? 0}</p></div></CardContent></Card>
      </div>

      {/* Revenue Stats */}
      {s.revenue && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">{pt("MRR")}</p><p className="text-2xl font-bold">{s.revenue.currency === 'EUR' ? '€' : '$'}{s.revenue.mrr?.toFixed(2) ?? '0.00'}</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">{pt("ARR")}</p><p className="text-2xl font-bold">{s.revenue.currency === 'EUR' ? '€' : '$'}{s.revenue.arr?.toFixed(2) ?? '0.00'}</p></CardContent></Card>
          {s.growth && <Card><CardContent className="p-4"><p className="text-sm text-muted-foreground">{pt("User Growth")}</p><p className="text-2xl font-bold flex items-center gap-1"><TrendingUp className="h-5 w-5 text-green-500" />{s.growth.users ?? 0}%</p></CardContent></Card>}
        </div>
      )}

      {(s.new_users || s.recent_users || s.recent_signups) && (
        <Card><CardHeader><CardTitle>{pt("Recent Users")}</CardTitle></CardHeader>
      ...
      {s.subscriptions_by_plan && s.subscriptions_by_plan.length > 0 && (
        <Card><CardHeader><CardTitle>{pt("Plan Distribution")}</CardTitle></CardHeader>
      ...
      <div className="space-y-4">
        <h2 className="text-lg font-semibold">{pt("Import & Export")}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <ProjectImport />
        </div>
        <TimesheetExport />
