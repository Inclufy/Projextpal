...
const WaterfallTesting = () => {
  ...
  const [form, setForm] = useState({ name: "", description: "", test_type: "unit", test_steps: "", expected_result: "" });
  ...
  const openCreate = () => { setEditing(null); setForm({ name: "", description: "", test_type: "unit", test_steps: "", expected_result: "" }); setDialogOpen(true); };
  const openEdit = (t: any) => { setEditing(t); setForm({ name: t.name || "", description: t.description || "", test_type: t.test_type || "unit", test_steps: t.test_steps || "", expected_result: t.expected_result || "" }); setDialogOpen(true); };
        ...
        {tests.length === 0 ? <Card className="p-8 text-center"><TestTube className="h-12 w-12 mx-auto text-muted-foreground mb-4" /><h3 className="text-lg font-semibold">No test cases yet</h3></Card> : (
          <div className="space-y-2">{tests.map(t => (
            <Card key={t.id}><CardContent className="p-4 flex items-center justify-between">
              <div className="flex-1"><div className="flex items-center gap-2 mb-1"><span className="font-medium">{t.name}</span><Badge variant="outline" className="text-xs">{t.test_type}</Badge><Badge className={`text-xs ${resultColors[t.status || "pending"]}`}>{t.status || "pending"}</Badge></div>{t.description && <p className="text-sm text-muted-foreground">{t.description}</p>}</div>
