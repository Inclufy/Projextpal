...
const AgileProductVision = () => {
  ...
  const saveGoal = async () => { if (!goalForm.title) { toast.error("Titel verplicht"); return; } try { const body: any = { title: goalForm.title, description: goalForm.description, priority: goalForm.priority, status: goalForm.status }; if (goalForm.target_date) body.target_date = goalForm.target_date; else body.target_date = null; const url = editingGoal ? `/api/v1/projects/${id}/agile/goals/${editingGoal.id}/` : `/api/v1/projects/${id}/agile/goals/`; const method = editingGoal ? "PATCH" : "POST"; const r = await fetch(url, { method, headers: jsonHeaders, body: JSON.stringify(body) }); if (r.ok) { toast.success("Opgeslagen"); setGoalDialog(false); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } };
