...
type Tailoring = {
  ...
  gov_authority: "owner" | "owner_pm"; gov_board: "auto" | "light" | "formal";
  gov_portfolio: boolean; gov_stakeholder_matrix: boolean; gov_periodic_cadence: boolean;
  shape: string; score: number; recommended_modules: string[];
  more?: string[]; methodology?: string; ai_rationale: string; source: string; coach_mode?: boolean;
};

const DIMS: { k: keyof Tailoring; label: string }[] = [
  { k: "dim_scope", label: "Omvang (aantal projecten)" },
...
const shapeColor: Record<string, string> = {
  light: "from-green-500 to-green-600", medium: "from-amber-500 to-amber-600", heavy: "from-red-500 to-red-600",
};

const ProgramTailoring = () => {
  ...
  const load = async () => {
    try {
      const r = await fetch(`/api/v1/programs/${id}/tailoring/`, { headers });
  ...
  const analyze = async () => {
    ...
    setAnalyzing(true);
    try {
      const res = await fetch(`/api/v1/programs/intake/analyze/`, { method: "POST", headers, body: JSON.stringify({ description: desc }) });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const r = await fetch(`/api/v1/programs/${id}/tailoring/`, { method: "PUT", headers, body: JSON.stringify(t) });
    ...
    <div className="min-h-full bg-background">
      <div className="p-6 space-y-6 max-w-5xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Programma-tailoring</h1>
            <p className="text-sm text-muted-foreground">Bepaal de juiste programmabesturing — niets wordt verborgen.</p>
          </div>
        ...
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /> Beschrijf het programma (AI vult de dimensies)</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <textarea className="w-full min-h-[70px] px-3 py-2 border rounded-md bg-background resize-y text-sm"
              placeholder="Bijv. Organisatiebrede transformatie met baten over meerdere jaren, directie betrokken."
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label className="text-xs">Aantal teams/projecten</Label>
                  <Seg value={t.team_size} onChange={(v) => set({ team_size: v })} options={[{ v: "s", label: "1–3" }, { v: "m", label: "4–9" }, { v: "l", label: "10+" }]} /></div>
                <div className="space-y-1.5"><Label className="text-xs">Betrokken organisaties</Label>
          ...
          <Card>
            <CardHeader><CardTitle className="text-base flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /> Governance (programmabestuur)</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-1.5"><Label className="text-xs">Wie autoriseert beslissingen?</Label>
                <Seg value={t.gov_authority} onChange={(v) => set({ gov_authority: v })} options={[{ v: "owner", label: "Programmabestuur" }, { v: "owner_pm", label: "Bestuur + manager" }]} />
                <p className="text-[11px] text-muted-foreground">{t.gov_authority === "owner" ? "Het programmabestuur autoriseert constituerende projecten." : "De programmamanager mag óók goedkeuren."}</p>
              </div>
              <div className="space-y-1.5"><Label className="text-xs">Formele Programme Board?</Label>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={`rounded-xl p-4 text-white bg-gradient-to-br ${shapeColor[t.shape] || shapeColor.medium}`}>
                <div className="text-[11px] uppercase tracking-wide opacity-85 font-bold">Programmavorm</div>
              ...
              <div className="md:col-span-2">
                ...
                {t.more && t.more.length > 0 && (
                  <>
                    <div className="text-[11px] uppercase tracking-wide text-muted-foreground font-bold mt-3 mb-2">Meer (optioneel)</div>
                    <div className="flex flex-wrap gap-1.5">
            ...
            {t.ai_rationale && (
              <div className="text-xs text-muted-foreground bg-muted/50 rounded-lg p-3">
                <span className="font-semibold text-foreground">Onderbouwing: </span>{t.ai_rationale}
              </div>
            )}
          </CardContent>
        </Card>
...
export default ProgramTailoring;
