...
const methodologyIcons: Record<string, { icon: string; label: string }> = {
  'hybrid': { icon: '🔀', label: 'Hybrid' },
  'msp': { icon: '⚙️', label: 'MSP' },
  'prince2': { icon: '👑', label: 'PRINCE2' },
  'safe': { icon: '⚡', label: 'SAFe' },
  'pmi': { icon: '📊', label: 'PMI' },
  'agile': { icon: '🔄', label: 'Agile' },
  'scrum': { icon: '🏃', label: 'Scrum' },
    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-violet-50 via-purple-50 to-fuchsia-50 dark:from-gray-900 dark:via-purple-900/20 dark:to-violet-900/20">
      ...
      <div className="relative z-10 p-8 md:p-10 space-y-8 max-w-[1800px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5">
          ...
          <StatCard title="Budget" value={formatBudget(totalBudget)} subtitle={t('app.totalAllocated')} icon={DollarSign} color="amber" />
                        ...
                        >
                          ...
                          <td className="py-4 px-6">
                            <span className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                              <span className="text-base">{methodInfo.icon}</span>
                          ...
                          <td className="py-4 px-6">
                            <span className="font-mono text-sm font-bold text-gray-900 dark:text-gray-100">{formatBudget(parseFloat(String(program.total_budget)) || 0)}</span>
                          </td>
                          <td className="py-4 px-6">
                  ...
                  <tbody className="bg-white dark:bg-gray-900 divide-y divide-purple-50 dark:divide-purple-900/20">
                    {formattedProjects.length > 0 ? formattedProjects.slice(0, 5).map((project) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <span className="font-mono text-sm font-bold text-gray-900 dark:text-gray-100">{formatBudget(parseFloat(String(project.budget)) || 0)}</span>
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
