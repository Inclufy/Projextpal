#!/bin/sh
set -e
# `set -x` echoes every command before running it — invaluable when the build
# log gives generic "exited with code 1" without telling you which line.
set -x
...
phase "ci_post_clone start (ProjeXtPal)"
date
sw_vers || true
