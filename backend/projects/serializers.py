...
class TaskSerializer(serializers.ModelSerializer):
    ...
    # ------------------------------------------------------------------
    # Yanmar push-back rule enforcement on due_date updates
    # ------------------------------------------------------------------
    def update(self, instance, validated_data):
        """
        Enforce Yanmar Project Plan rule on due_date push-backs:

          "Due dates can only be pushed back once for a maximum period of
           2 weeks. More than this should be discussed with and approved
           by project owner."

        Implementation:
        - 1st push-back AND delta <= 14 days  -> apply, bump revision_count,
          set revised_due_date to the new value, keep due_date as the
          original baseline.
        - 2nd+ push-back OR delta > 14 days   -> create a
          TaskDueDateChangeRequest (status=pending) and DO NOT change the
          stored due_date. Caller receives 202 with the request id via
          the view (we attach `_pending_due_request` on the instance so
          the view can pick it up).
        - Pull-forward (delta <= 0) is always allowed without approval.
        """
        ...
        if new_due and instance.due_date and new_due != instance.due_date:
            ...
            if delta > 0:  # push-back, not pull-forward
                needs_approval = (
                    instance.revision_count >= 1 or delta > 14
                )
                if needs_approval:
                    request = self.context.get("request")
                    user = request.user if request else None
                    pending = TaskDueDateChangeRequest.objects.create(
                        task=instance,
                        requested_by=user if user and user.is_authenticated else None,
                        requested_date=new_due,
                    ...
                    instance._pending_due_request = pending
...
class TaskDueDateChangeRequestSerializer(serializers.ModelSerializer):
    ...
    decided_by_email = serializers.ReadOnlyField(source="decided_by.email")
    task_title = serializers.ReadOnlyField(source="task.title")
    project_id = serializers.ReadOnlyField(source="task.milestone.project_id")

    class Meta:
        from .models import TaskDueDateChangeRequest
        model = TaskDueDateChangeRequest
        fields = [
            "id", "task", "task_title", "project_id",
            "requested_by", "requested_by_email",
            "requested_date", "previous_due_date", "delta_days",
            "reason", "status",
        ...
        read_only_fields = [
            "id", "requested_by", "previous_due_date", "delta_days",
